...
RegisteredTileBag::RegisteredTileBag(const Tile& origin) :
    ...
    predecessors()
{
    // EWOK_STL's std::list has no initializer_list constructor, so seed the
    // origin tile imperatively rather than with upstream's
    // `byBestCostTiles({ &origin })`.
    byBestCostTiles.push_back(&origin);
}
