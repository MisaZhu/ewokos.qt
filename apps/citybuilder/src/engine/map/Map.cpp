...
void Map::createBuilding(const BuildingInformation& conf, const TileCoordinates& leftCorner, Direction orientation)
{
    ...
    if (conf.getType() == BuildingInformation::Type::Road) {
        ...
        // Port note: upstream wrapped this in try/catch (NotImplementedException)
        // around the entry-point lookup.  With -fno-exceptions the lookup now
        // returns nullptr when the building has no road entry point, so the
        // recoverable "skip the creation" path becomes an explicit check.
        auto entryPoint(getBestBuildingEntryPoint(area));
...
Tile* Map::getBestBuildingEntryPoint(const TileArea& area) const
{
    ...
    while (!tile || !tile->isRoad()) {
        ...
        if (moveX == 1 && x > right.x()) {
            ...
            // Overstep left corner. No road node found: signal "no entry point"
            // with a null return rather than an exception (-fno-exceptions).
            return nullptr;
        }
