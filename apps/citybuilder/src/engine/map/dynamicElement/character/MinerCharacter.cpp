        ...
        if (goingHome) {
            ...
            if (workingCountDown == conf.getActionInterval()) {
                ...
                if (target.isNull()) {
                    ...
                    (void) NotImplementedException("Handling destroyed natural resource");
                }
                auto naturalResource(target->asNatureElement());
                assert(naturalResource != nullptr);
                if (naturalResource->isBusy()) {
                    ...
                    if (path.isNull()) {
                        ...
                        (void) NotImplementedException("Destroy character");
                    }
                    motionHandler.takePath(path);
                    return;
                }
