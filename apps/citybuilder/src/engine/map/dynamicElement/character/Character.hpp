...
class Character : public AbstractProcessable
{
    ...
    public:
        ...
        // Port note (EwokOS, -fno-rtti): upstream told a DeliveryManCharacter /
        // ImmigrantCharacter apart from a plain Character with dynamic_cast in
        // the buildings' processInteraction().  dynamic_cast on a polymorphic
        // type needs RTTI, which is disabled here, so each concrete character
        // reports its kind through this virtual and callers static_cast after
        // checking it.  Kind::Base means "none of the cast targets", exactly
        // what a failed dynamic_cast used to yield.
        enum class Kind { Base, DeliveryMan, Immigrant };
        virtual Kind getKind() const { return Kind::Base; }
