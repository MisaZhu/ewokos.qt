...
QWeakPointer<AbstractStaticElement> MotionHandler::target() const
{
    ...
    auto targetedPath(path->asTargetedPath());
    if (targetedPath == nullptr) {
        return {};
    }
...
Direction MotionHandler::resolveDirection() const
{
    ...
    (void) UnexpectedException("Could not resolve direction of a motion from " + movingFrom->coordinates().hash() + " to " + next.hash());
}
