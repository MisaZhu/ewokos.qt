...
const ModelReader ModelReader::getSubModel(const char key[]) const
{
    if (!node[key] || !node[key].IsMap()) {
        (void) BadConfigurationException(generateErrorMessage(key, "an object"));
    }
...
{
    if (!node[key]) {
        (void) BadConfigurationException(generateErrorMessage(key, "a number"));
    }
...
{
    if (!node[key]) {
        (void) BadConfigurationException(generateErrorMessage(key, "a string"));
    }
...
{
    if (!node[key]) {
        (void) BadConfigurationException(generateErrorMessage(key, "some point coordinates"));
    }
...
QList<QPoint> ModelReader::getPointList(const char key[]) const
{
    if (!node[key] || !node[key].IsSequence()) {
        (void) BadConfigurationException(generateErrorMessage(key, "a list of point coordinates"));
    }
...
{
    if (!node[key]) {
        (void) BadConfigurationException(generateErrorMessage(key, "a list of items"));
    }
