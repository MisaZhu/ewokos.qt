...
{
    if (!model["title"]) {
        (void) BadConfigurationException("Missing \"title\" parameter in configuration for node \"" + key + "\".");
    }
    if (!model["traversable"]) {
        (void) BadConfigurationException("Missing \"traversable\" parameter in configuration for node \"" + key + "\".");
    }
}
...
NatureElementInformation::Type NatureElementInformation::resolveType(const QString& type)
{
    ...
    (void) BadConfigurationException("Unknown nature element of type \"" + type + "\".");
}
