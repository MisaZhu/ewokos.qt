...
const BuildingInformation& ControlPanelElementInformation::getStaticElementConf() const
{
    if (!staticElementConf) {
        (void) UnexpectedException("This building conf does not have house information.");
    }
...
{
    if (!model["type"]) {
        (void) BadConfigurationException("Missing \"type\" parameter in configuration for control panel element.");
    }
    if (!model["title"]) {
        (void) BadConfigurationException("Missing \"title\" parameter in configuration for control panel element.");
    }
}
...
ControlPanelElementInformation::Type ControlPanelElementInformation::resolveType(const QString& type)
{
    ...
    (void) BadConfigurationException("Unknown control panel element of type \"" + type + "\"");
}
