...
Simulator::~Simulator()
{
    // Upstream waits for the worker thread here.  There is no worker thread -
    // see the note on the removed include in simulator.h - and runCircuit()
    // returns before the call that started it returns, so there is nothing that
    // could still be running when the Simulator goes away.
}

inline void Simulator::solveMatrix()
{
    for (eNode *node : m_eChangedNodeList)
        node->stampMatrix();
    ...
    QTimerEvent *e) // update at m_timerTick rate (50 ms, 20 Hz max)
{
    ...
    // Upstream spends this block bringing the previous tick's worker thread to a
    // halt before starting another one, so that two runCircuit() calls are never
    // in the circuit at once.  Running it here instead makes that structurally
    // impossible rather than something to be checked for.
    ...
    // On the GUI thread, so this blocks the redraw for as long as the batch of
    // m_circuitRate steps takes.  Upstream overlaps the two and pays for it with
    // the synchronisation above; the trade here is that a circuit slow enough to
    // overrun a tick drops frames instead of falling behind and then being
    // waited for, and the cost is a Qt module this build does not have.
    runCircuit();
...
void Simulator::runGraphicStep2()
{
    ...
    if (m_debugging)
        m_tStep = m_step; // Debugger driving, not timer
    ...
    if (deltaRefTime >= 1e9) // We want steps per Sec = 1e9 ns
    {
        ...
        CircuitWidget::self()->setRate((stepsPerSec * 100) /
                                       (1e6 * m_stepsPerus) /*m_simuRate*/);
        m_lastStep    = m_tStep;
...
void Simulator::runCircuit()
{
    ...
    for (uint64_t i = 0; i < m_circuitRate; i++) {
        ...
        if (!m_nonLinear.isEmpty()) // Run Non-Linear elements
        {
            ...
            // if( counter > 0 ) qDebug() << "\nSimulator::runCircuit Non-Linear
            // Solved in steps:"<<counter;
        }
        if (!m_eChangedNodeList.isEmpty())
            solveMatrix();
        ...
        m_step++;
        if (!m_isrunning)
            break; // Keep this at the end for debugger to run 1 step
    }
    /*uint64_t time = m_RefTimer.nsecsElapsed()-time0;
    count++;
    circTime += time;*/
    // if( count == 1000 )
    // qDebug() << "Simulator::runCircuit Time nS:"<<time<<circTime<<count <<
    // circTime/count;
}
...
void Simulator::startSim()
{
    ...
    for (eNode *busNode : m_eNodeBusList)
    ...
    std::cout << "  Initializing " << m_eNodeBusList.size() << "\tBuses"
              << std::endl;
    for (eNode *busNode : m_eNodeBusList)
    ...
    if (McuComponent::self() && !m_paused)
        McuComponent::self()->runAutoLoad();
...
int Simulator::simuRateChanged(int rate)
{
    ...
    std::cout << "\nFPS:              " << fps
              ...
              << "\nReactive SubRate: " << m_stepsPrea << std::endl
              << std::endl;
...
{
    if (!m_reactive.contains(el))
        m_reactive.append(el);
}
