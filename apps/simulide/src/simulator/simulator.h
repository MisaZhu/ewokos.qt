...
// Upstream also includes <qtconcurrentrun.h> here and holds a QFuture<void>
// m_CircuitFuture below, because it runs runCircuit() on a worker thread and
// overlaps it with the redraw.  This Qt build was configured without the
// Concurrent module - QT_FEATURE_concurrent is -1 in qconfig.h and there is no
// libQt5Concurrent.a - so the step loop runs on the GUI thread instead.  See
// Simulator::timerEvent() for what that changes.
