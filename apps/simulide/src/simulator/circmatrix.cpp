...
// <cmath> and not upstream's <math.h>.  luSolve() below calls std::isnan(volt),
// which is correct C++ and fails only here, because this SDK's math.h defines
// isnan as a macro:
//
//     #define isnan(x) __builtin_isnan(x)          math.h:13
//
// A macro does not know about qualification, so it expands inside the std:: and
// GCC is left looking at `std::__builtin_isnan(volt)` - reported as
// `'__builtin_isnan' is not a member of 'std'; did you mean '__builtin_isnan'?`,
// with the error attributed to math.h:13 rather than to the line that called it.
//
// The SDK's own <cmath> exists to prevent exactly this.  It documents the trap,
// `#undef`s isnan, and then supplies three real inline overloads - float, double,
// long double - that call the same builtin as functions rather than as a macro,
// and exports them with `using std::isnan`.  Including it fixes the call site
// without touching it, which is the point: std::isnan(volt) is what upstream
// wrote and it is right.
...
{
    if (!m_admitChanged && !m_currChanged)
        return true;
    ...
    if (m_circChanged) // Split Circuit into unconnected parts
    {
        ...
        while (
            !allNodes.isEmpty()) // Get a list of groups of nodes interconnected
        {
            QList<int> nodeGroup;
            addConnections(allNodes.first(), &nodeGroup,
                           &allNodes); // Get a group of nodes interconnected
            ...
            if (numEnodes == 1) // Sigle nodes do by themselves
            {
                ...
                enod->setSingle(true);
                enod->solveSingle();
                // qDebug() <<"CircMatrix::solveMatrix solve
                // single"<<enod->itemId();
            } else {
                dp_matrix_t a;
                d_matrix_t ap;
                ...
                int ny = 0;
                for (int y = 0; y < m_numEnodes;
                     y++) // Copy data to reduced Matrix
                {
                    if (!nodeGroup.contains(y + 1))
                        continue;
                    int nx = 0;
                    for (int x = 0; x < m_numEnodes; x++) {
                        if (!nodeGroup.contains(x + 1))
                            continue;
                        a[nx][ny] = &(m_circMatrix[x][y]);
                        // qDebug() <<"CircMatrix::solveMatrix
                        // cell"<<nx<<ny<<*(a[nx][ny]);
                        nx++;
                    }
                    b[ny] = &(m_coefVect[y]);
...
void CircMatrix::factorMatrix(int n, int group)
{
    ...
    {
        for (i = 0; i < j;
             i++) // calculate upper triangular elements for this column
        {
            double q = a[i][j];
            for (k = 0; k < i; k++)
                q -= a[i][k] * a[k][j];
        ...
        for (i = j; i < n; i++) {
            ...
            // qDebug() <<"LTE"<<i<<j<<x<<q<<largest<<largestRow<<( !(x <
            // largest) );
        }
...
bool CircMatrix::luSolve(int n, int group)
{
    ...
    for (/*i = bi*/; i < n; i++) {
        ...
        for (int j = bi; j < i; j++)
            tot -=
                a[i][j] *
                b[j]; // forward substitution using the lower triangular matrix
