...
//qDebug() <<"Pin::registerPinsW "<<m_component->itemID();
    ...
    if( m_component->itemType() == "Node" )
    {
        ...
        // -fno-rtti, so dynamic_cast to a class type is a hard error rather than a
        // warning: `'dynamic_cast' not permitted with '-fno-rtti'`.  RTTI cannot
        // simply be switched on either - Qt is built without it as well, and a
        // dynamic_cast across a boundary where one side has no type_info would
        // fail at run time instead of at compile time, which is worse.
        //
        // qobject_cast is not a compromise here.  Component derives from QObject
        // (component.h:34, `public QObject, public QGraphicsItem`) and both Node
        // and Bus carry Q_OBJECT, so this resolves through the meta-object system
        // rather than through type_info - it needs no RTTI, it is a couple of
        // pointer comparisons against staticMetaObject rather than a walk of the
        // inheritance graph, and it answers the same question.  The enclosing
        // itemType() test has already established what m_component is, so neither
        // cast could have returned null in any case.
        Node* node = qobject_cast<Node*>(m_component);
        node->registerPins( enode );
    }
    else if( m_component->itemType() == "Bus" )
    {
        Bus* bus = qobject_cast<Bus*>(m_component);
