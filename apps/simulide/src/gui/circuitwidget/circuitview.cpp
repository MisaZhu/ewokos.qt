...
// Upstream has `#include <QSvgGenerator>` here.  QtSvg is not in this port and
// cannot be: it is its own upstream repository, not a module of qtbase, and
// there is no copy of it anywhere in this tree - `find` for qtsvg turns up
// nothing.  Only one function used it, saveImage() below, and only for one of
// the five formats it offered, so the include goes with the branch rather than
// being stubbed.  A stub QSvgGenerator would have to be a QPaintDevice that
// records drawing into an SVG document, which is not a stub, it is QtSvg.
...
void CircuitView::saveImage()
{
    ...
    // "SVG (*.svg);;" is out of the filter.  Leaving it in would offer a format
    // that then silently saved a raster image under an .svg name - the
    // endsWith() test below is what routed .svg to QSvgGenerator, and without
    // that branch such a file falls through to QPixmap::save, which picks its
    // writer from the suffix and would find none for svg.  Dropping the entry
    // is the smaller lie.
    QString fileName = QFileDialog::getSaveFileName( this
                            , tr( "Save as Image" )
                            , circPath
                            , "PNG (*.png);;JPEG (*.jpeg);;BMP (*.bmp);;All (*.*)"  );
    if (!fileName.isNull())
    {
