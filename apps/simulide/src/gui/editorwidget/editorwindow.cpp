...
{
    // qobject_cast, where upstream has dynamic_cast.  This port builds with
    // -fno-rtti, so dynamic_cast to a class type is a hard error and not a
    // warning: `'dynamic_cast' not permitted with '-fno-rtti'`.  Turning RTTI
    // back on is not the alternative it looks like - Qt itself is built without
    // it, so a cast that crossed into a Qt type would compile here and then fail
    // at run time for want of type_info, which is a worse place to find out.
    //
    // qobject_cast is not a compromise either.  CodeEditorWidget derives from
    // QWidget and carries Q_OBJECT (codeeditorwidget.h:30,32), so this resolves
    // through the meta-object system instead of through type_info: two pointer
    // comparisons against staticMetaObject rather than a walk of the inheritance
    // graph, and it returns null on a miss exactly as dynamic_cast does.  The
    // `if (actW)` below therefore means the same thing it meant upstream.
    //
    // The same substitution appears in closeTab(), and pin.cpp has two more; all
    // four are the only dynamic_casts in the tree.
    CodeEditorWidget *actW =
        qobject_cast<CodeEditorWidget *>(m_docWidget->currentWidget());
    if (actW)
...
void EditorWindow::closeTab(int index)
{
    ...
    CodeEditorWidget *actW =
        qobject_cast<CodeEditorWidget *>(m_docWidget->widget(index));
