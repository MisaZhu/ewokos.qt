...
{
    // Everything from here to the #else below is upstream's, unedited, and it is
    // fenced off rather than deleted because it cannot compile on this platform
    // and never will be able to.
    //
    // QPluginLoader loads a shared object at run time, and run-time loading needs
    // dlopen.  This SDK has no dlfcn.h and no libdl.a - Qt is configured with
    // -no-feature-dlopen for exactly that reason - so QT_CONFIG(library) is false
    // and QtCore's qpluginloader.h takes its #else branch, which declares a class
    // with two static members and nothing else:
    //
    //     class Q_CORE_EXPORT QPluginLoader
    //     { public: static QObjectList staticInstances();
    //               static QVector<QStaticPlugin> staticPlugins(); };
    //
    // No constructor from a QString, no instance(), no unload(), no
    // errorString().  That is why the diagnostics here read "has no member named"
    // instead of "was not declared": the name exists and is a complete type, it is
    // just not the class this code was written against.
    //
    // Which also rules out the fix used for QProcess.  src/compat/ can supply a
    // QProcess because Qt supplies none; it cannot supply a QPluginLoader because
    // Qt supplies one, and a second definition of a complete class in the same
    // translation unit is itself an error.  Nor can the feature be enabled the way
    // QtXml was - that module was sitting in qtbase merely switched off, whereas
    // dlopen is a facility of the kernel and libc that this system does not have.
    //
    // It would find nothing in any case.  SimulIDE is linked statically and every
    // component is already inside the executable, so the data/plugins directory
    // this walks does not exist in the installed tree.
    //
    // Guarded rather than removed so that a diff against upstream still shows the
    // original function, and so that it reinstates itself if the platform ever
    // grows a dynamic loader.
    ...
    qDebug() << "\n";
#else
    // loadPlugins() at line 70 and componentselector.cpp:435 both still call
    // this, and the menu item behind the latter is real, so the function has to
    // exist.  Returning quietly is the honest behaviour: nothing could be loaded,
    // and a dialog on every start-up would report a fact about the platform as
    // though it were a failure of the user's.
    Q_UNUSED( pluginsDir )
#endif
}
...
{
    // Guarded for the reason set out in loadPluginsAt() above.  m_plugins is only
    // ever filled there, so on this platform the hash stays empty and the body
    // would not execute even if it compiled.
#if QT_CONFIG(library)
    if (m_plugins.contains(pluginName)) {
