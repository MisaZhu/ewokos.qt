...
void MainWindow::handleHistory()
{
    ...
    args.removeFirst();
    args << fn;
    // EwokOS: Qt is built -no-feature-process, so QProcess::startDetached is
    // unavailable.  Detach the way xfinder launches apps: fork, detach from
    // the parent, exec the command line.
    QByteArray cmdLine = (command + QLatin1Char(' ') + args.join(QLatin1Char(' '))).toLocal8Bit();
    int pid = fork();
    if (pid == 0) {
        proc_detach();
        proc_exec(cmdLine.constData());
        _exit(-1);
    }
    if (pid < 0) {
