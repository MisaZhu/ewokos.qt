...
/* EwokOS: the same reason as macOS.  The Qt font database here is the FreeType
   one with no fontconfig, so the generic "Monospace" alias is never mapped to a
   fixed-pitch face - the terminal silently falls back to the proportional theme
   font and every column of tabular output is misaligned.  Name a real monospace
   TTF that ships in /usr/system/fonts (family as FreeType reads it from the
