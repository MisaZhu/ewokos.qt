...
QList<QString> ColorSchemeManager::listColorSchemes()
{
    ...
    for (const QString &scheme_dir : get_color_schemes_dirs())
    {
        ...
        for (const QString &i : list)
            /* EwokOS: dir.filePath() rather than dname + '/' + i.  The dirs
               from get_color_schemes_dirs() already carry a trailing '/', so
               concatenating another one yields ".../color-schemes//Foo.colorscheme".
               A double slash mid-path is legal on POSIX but EwokOS's VFS does
               not collapse it, so QFile::exists() below rejects every scheme
               and all of them fail to load.  filePath() joins cleanly. */
            ret << dir.filePath(i);
    }
    return ret;
