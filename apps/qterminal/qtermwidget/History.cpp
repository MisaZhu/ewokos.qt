...
void HistoryScrollBuffer::setMaxNbLines(unsigned int lineCount)
{
    ...
    /* EwokOS -fno-rtti: was an unchecked dynamic_cast dereference; the only
       HistoryType that owns a HistoryScrollBuffer is HistoryTypeBuffer. */
    static_cast<HistoryTypeBuffer*>(m_histType)->m_nbLines = lineCount;
}
...
HistoryScroll* HistoryTypeBuffer::scroll(HistoryScroll *old) const
{
  if (old)
  {
    /* EwokOS -fno-rtti: kind() replaces dynamic_cast<HistoryScrollBuffer*>. */
    HistoryScrollBuffer *oldBuffer = (old->kind() == HistoryScroll::KindBuffer)
        ? static_cast<HistoryScrollBuffer*>(old) : nullptr;
    if (oldBuffer)
    {
...
{
  /* EwokOS -fno-rtti: upstream wrote dynamic_cast<HistoryFile*>(old), and
     HistoryFile is not a HistoryScroll subclass, so that check never matched
     and every call rebuilt the scroll.  kind() == KindFile is what it was
     reaching for; keeping the file-backed scroll on a same-type switch is
     safe because the log file name is fixed per session. */
  if (old != nullptr && old->kind() == HistoryScroll::KindFile)
...
HistoryScroll* CompactHistoryType::scroll ( HistoryScroll *old ) const
{
  if ( old )
  {
    /* EwokOS -fno-rtti: kind() replaces dynamic_cast<CompactHistoryScroll*>. */
    CompactHistoryScroll *oldBuffer = (old->kind() == HistoryScroll::KindCompact)
        ? static_cast<CompactHistoryScroll*>(old) : nullptr;
    if ( oldBuffer )
    {
      oldBuffer->setMaxNbLines ( m_nbLines );
      return oldBuffer;
