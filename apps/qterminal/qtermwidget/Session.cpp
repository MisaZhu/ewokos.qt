...
//    QDBusConnection::sessionBus().registerObject(QLatin1String("/Sessions/")+QString::number(_sessionId), this);
    ...
    // EwokOS: the shell runs on a pipe pair, there is no pty slave fd.
    ptySlaveFd = -1;
...
//            SLOT( fireZModemDetected() ) );
    ...
    connect( _shellProcess, &Pty::finished, this, &Session::done );
...
{
    return _shellProcess->state() == Pty::Running;
}
...
void Session::run()
{
    ...
    {
        /* EwokOS: the last-resort shell.  Upstream picks /bin/sh on the
           assumption it exists on every Unix; EwokOS has no /bin/sh - its only
           shell is /bin/shell, the same one sshd/telnetd spawn and that Pty.h
           documents.  $SHELL is normally unset here, so without this the exec
           below targets a path that does not exist, the session ends at once
           and qterminal quits with its last window. */
        const QString defaultShell{QLatin1String("/bin/shell")};
        ...
        if ( exec.isEmpty() || !excheck.exists() ) {
            exec = defaultShell;
            /* EwokOS: $SHELL is normally unset, so arriving here is the
               expected path, not a fault - the fallback /bin/shell is correct
               and the session runs.  Only warn when even that is missing,
               which is the sole case where the exec below cannot succeed. */
            excheck.setFileName(exec);
            if ( !excheck.exists() )
...
void Session::close()
{
    _autoClose = true;
    _wantedClose = true;
    // EwokOS delivers no SIGHUP; SYS_SIG_KILL is the only terminate signal.
    if (!_shellProcess->isRunning() || !sendSignal(SIGKILL)) {
        // Forced close.
...
void Session::done(int exitCode, Konsole::Pty::ExitStatus exitStatus)
{
    if (!_autoClose) {
    ...
    if (!_wantedClose || exitCode != 0) {
        ...
        if (_shellProcess->exitStatus() == Pty::NormalExit) {
    ...
    if ( !_wantedClose && exitStatus != Pty::NormalExit )
